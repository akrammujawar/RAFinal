...
const Pagination = ({ orgData, setNewFilterarr }: appProps) => {
  ...
  const handleNext = (newFilterarr: any) => {
    ...
    if (currPage < pageCount - 1) {
      ...
      //     setEndPos(endPos + rowSize)
      // }
      orgData?.length > endPos && setStartPos(startPos + rowSize);
      setEndPos(endPos + rowSize);
    } else {
        ...
        <div className="footer fixed-bottom">
          ...
          >
            <div>
              ...
              {rowSize === null ? (
                <span className="ps-2 opacity-50 cursor-pointer">
                  entries | Showing {startPos + 1} to {orgData?.length} of{" "}
                  {orgData?.length} entries
                </span>
              ) : (
                <span className="ps-2 opacity-50 cursor-pointer">
                  entries | Showing {startPos + 1} to{" "}
                  {endPos > orgData?.length ? orgData?.length : endPos} of{" "}
                  {orgData?.length} entries
                </span>
              )}
            </div>
            <div>
              <span
                className={`${
                  startPos < 1 ? "opacity-50 cursor-pointer" : ""
                }${"me-1 pointer cursor-pointer"}`}
                onClick={() => handlePrev()}
              >
                Previous
              </span>
              ...
              {orgData && rowSize ? (
                [...Array(Math.ceil(orgData?.length / rowSize))]?.map(
                  (itr: any, i: any) => (
                    <span
                      key={i}
                      className={`${
                        currPage === i
                          ? "border border-dark cursor-pointer"
                          : "opacity-50 cursor-pointer"
                      } ${"mx-1 px-3 py-2 pointer"}`}
                      onClick={() => {
                        handlePageNavigation(i + 1), setCurrPage(i);
                      }}
                    >
                      {i + 1}
                    </span>
