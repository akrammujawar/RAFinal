...
const Allocation: React.FunctionComponent<IAllocatorProps> = (props: any) => {
  ...
  const [showProjectAllocation, setShowProjectAllocation] =
  ...
  async function getProjectAllocationListData(year: any) {
    try {
      let loggedUser = await _sharePointServiceProxy
        .getCurrentUser()
        .then((res: any) => {
          return res;
        });
      ...
      if (loggedUser.Groups.length === 0) {
        ...
        let LoginCurrentUser = projectListItems.filter((i: any) => {
          return (
            i?.Manager1?.Title === loggedUser?.User?.Title ||
            i?.EmployeeId?.EmpEmail === loggedUser?.User?.Email
          );
        });
        setProjectsAllocation(LoginCurrentUser);
  ...
  const columnDefs: any = [
      ...
      children: [
        {
          ...
          editable:
            ...
            (currentUser &&
              currentUser.Groups &&
              currentUser.Groups.length > 0 &&
              currentUser.Groups[0].Title === "RA_Manager")
              ? false
              : true,
        ...
        },
        {
          ...
          singleClickEdit:
            (currentUser &&
              currentUser.Groups &&
              currentUser.Groups.length > 0 &&
              currentUser.Groups[0].Title === "RA_Reader") ||
            (currentUser &&
              currentUser.Groups &&
              currentUser.Groups.length > 0 &&
              currentUser.Groups[0].Title === "RA_Manager")
    ...
    },
    {
      ...
      cellRendererParams: {
        ...
        webURL: props?.webURL,
        getProjectAllocationListData: getProjectAllocationListData,
      },
      width: 120,
    },
  ];
  ...
  async function weakCellValueChanged(params: any) {
    let updateColumnValue =
      params.column.originalParent.colGroupDef.headerName.split(" ")[0];
  ...
  function getEmployeeallocatedwithProject(ProjectName: any) {
    let getemployeewithallocatedProject = ProjectsAllocation.filter(
      (ftr) => ftr?.Project_ID?.ProjectName === ProjectName
    ).map((itr) => ({
  ...
  async function addAllocateResource() {
    if (validate()) {
      let isemployeeallocatedwithsameproject =
        getprojectEmp.filter((ftr: any) => ftr?.EmployeeName === getselectedEmp)
          .length > 0;
      if (isemployeeallocatedwithsameproject) {
        ...
        await _sharePointServiceProxy
          .addItem("ProjectsAllocations", ProjectWithEmployee, [], true)
          .then(() => {
            setGlobalMsg(false);
            setProjectWithEmployee({
              Project_IDId: "",
  ...
  return (
    <>
      ...
      <Modal
        isOpen={show}
        onDismiss={() => { setShow(false); setSelectedProject(null); setSelectedEmployee(null); }}
        isBlocking={true}
            ...
            <form>
              <div className="modal-body">
                <div className="row g-3">
                  <div className="col-md-6">
                    ...
                    <Select
                      ...
                      onChange={(selected: any) => {
                        ...
                        setProjectWithEmployee({
                          ...ProjectWithEmployee,
                          Project_IDId: selected?.value,
                        });
                        getEmployeeallocatedwithProject(selected?.label);
                      }}
                  ...
                  <div className="col-md-6">
                    ...
                    <Select
                      ...
                      onChange={(selected: any) => {
                        ...
                        setProjectWithEmployee({
                          ...ProjectWithEmployee,
                          EmployeeIdId: selected?.value,
                        });
                        setselectedEmp(selected?.label);
                      }}
                      placeholder="Search or select..."
                      isSearchable
                    />
                  ...
                  <div className="col-md-6">
                    ...
                    />
                    {!ProjectWithEmployee?.Manager1Id && (
                      <p
                        className={`${
                          globalMsg
                            ? "d-block text-danger mb-0 error-feild-size"
                  ...
                  <div className="col-md-6">
                    ...
                    <PeoplePicker
                      ...
                      principalTypes={[PrincipalType.User]}
                    />
                    {!ProjectWithEmployee?.Manager2Id && (
                      <p
                        className={`${
                          globalMsg
                            ? "d-block text-danger mb-0 error-feild-size"
              ...
              <div className="d-flex justify-content-end mt-3">
                <button
                  ...
                  onClick={() => {
                    setShow(false); setGlobalMsg(false); setUpdateModal(""); setSelectedProject(null); setSelectedEmployee(null);
                  }}
                >
                  Cancel
                </button>
          ...
          <div className="row mt-4 pt-2 mx-0">
            ...
            <div className="col-md-6">
              <div className="d-flex">
                ...
                <div>
                  <h4 className="pt-2 ms-2">Resource Allocation</h4>
                  <h3 className="bredcram-subhead ms-2">
                    <span
                      className="text-primary cursor-pointer"
                      onClick={() => navigate("")}
                    >
                      <a href="https://bluebenz0.sharepoint.com/">Dashboard</a>
                    </span>
                    <span> / </span>
                    <span
            ...
            {showProjectAllocation && (
              <div className="col-md-6 p-1 text-end">
                <button className="btn btn-primary mt-2" onClick={onBtnExport}>
                    ...
                    <div className="card-body ">
                      <div className="align-items-center d-flex justify-content-end">
                        <select
                          ...
                          onChange={(e) => {
                            getProjectAllocationListData(e.target.value);
                        ...
                        {showProjectAllocation && (
                          <svg
                            onClick={() => {
                              Number(yearValue) !== new Date().getFullYear()
                                ? alert(
                                    "You cannot allocate project in the previous year."
                                  )
                                : setShow(true);
                            }}
